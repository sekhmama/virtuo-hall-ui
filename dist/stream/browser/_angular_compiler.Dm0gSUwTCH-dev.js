...
var BoundDeferredTrigger = class extends DeferredTrigger {
  constructor(value, sourceSpan, prefetchSpan, whenSourceSpan) {
    super(/** nameSpan */
    null, sourceSpan, prefetchSpan, whenSourceSpan);
    this.value = value;
  }
};
...
var I18nMetaVisitor = class {
  ...
  _setMessageId(message, meta) {
    if (!message.id) {
      message.id = meta instanceof Message && meta.id || decimalDigest(message, /* preservePlaceholders */
  ...
  _setLegacyIds(message, meta) {
    if (this.enableI18nLegacyMessageIdFormat) {
      message.legacyIds = [computeDigest(message), computeDecimalDigest(message, /* preservePlaceholders */
      this.preserveSignificantWhitespace)];
...
${errors.join("\n")}`);
  ...
  digest(message) {
    return digest(message, /* preservePlaceholders */
    true);
  }
  createNameMapper(message) {
    return new SimplePlaceholderMapper(message, toPublicName);
...
function createSerializer(format) {
  ...
  switch (format) {
    case "xmb":
      return new Xmb(/* preservePlaceholders */
      true);
    case "xtb":
      return new Xtb();
    case "xliff2":
